...
module.exports = [
	...
	},
	{
		...
		value:
			'!f() { for var in "$@"; do git branch -D $var & git push origin :$var; done }; f'
	},
	{
		key: "please",
	...
	},
	{
		...
		desc: "pretty log",
		value:
			'log --pretty=format:"%C(yellow)%h %Cblue%>(12)%cr %Cgreen%<(20)%aN%Cred%d %Creset%s" --graph'
	},
	{
		key: "pruner",
	...
	},
	{
		...
		value:
			'!f() { git checkout -b $(date +%Y-%m-%d)-$(echo $@ | tr " " "-"); }; f'
	},
	{
		key: "get",
		desc: "start a repo by remote URL",
		value:
			"!f() { git init; git remote add origin $1; git pull origin master; }; f"
	},
	{
		key: "from",
	...
	},
	{
		...
		value:
			'!f() { git commit --amend -m "$(curl -s whatthecommit.com/index.txt)"; }; f'
	},
	{
		key: "wip",
		desc:
			"add everything, commit with a random commit message and push to remote origin",
	...
	},
	{
		...
		value:
			"!f() { git checkout master && git pull origin master && git checkout - && git rebase master; }; f"
	}
];
