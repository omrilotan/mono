...
async function app() {
	...
	aliases = aliases
		...
		.filter(({ key, value }) => showAll || existing[key] !== value)

		// Warn about existing but different
		.map(({ key, desc, value, disabled = false }) => {
	...
	const choices = aliases.map(({ key, desc, value, disabled }) => ({
	...
	hazard &&
		message.push("[☠️ ] marks an alias you have with a different value".dim);
	match &&
		message.push("[🎀️ ] marks an alias you have with the same value".dim);
	message.push("\t");
	...
	switch (selected.length) {
		...
		default:
			return `I've set up these aliases for you: ${selected
				.map(([key]) => `"${key.bold}"`)
				.join(", ")} 😃`;
	}
}
