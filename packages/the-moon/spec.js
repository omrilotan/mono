...
const { ICONS, CODES, NAMES } = moon;

describe("moon", async () => {
	it("Should return a moon phase", () => expect(moon()).to.be.oneOf(ICONS));
	...
	[
		...
		it(`Should return a moon phase name (${args
			.map(JSON.stringify)
			.join(", ")})`, () => expect(moon(...args)).to.be.oneOf(CODES))
	);
	...
	[
		...
		it(`Should moon phase for ${[year, month, date].join(
			"-"
		)} is ${result}`, () =>
	...
	[["icon", ICONS], ["code", CODES], ["name", NAMES]].forEach(
		([format, array]) =>
			it(`Should return a format:${format}`, () =>
