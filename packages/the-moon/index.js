const { MOON_CYCLE, YEAR_CYCLE, MONTH_CYCLE } = require("./lib/cycles");
const {
	FORMAT_ICON,
	FORMAT_CODE,
	FORMAT_NAME,
...
const { ICONS, CODES } = require("./lib/names");
...
function moon(...args) {
	...
	let total_days_elapsed =
		year * YEAR_CYCLE + month * MONTH_CYCLE + day - NINETEEN_HUNDRED;
...
module.exports = Object.defineProperties(moon, {
	ICONS: {
		value: ICONS,
		enumerable: true
	},
	...
	NAMES: {
		get: () => CODES.map(name => [word(name), SUFFIX].join(" ")),
		enumerable: true
	},
	FORMATS: {
		value: FORMATS,
