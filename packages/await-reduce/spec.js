...
describe("await-reduce", async () => {
	...
	it("Should use first items value as default initial value", async () =>
		expect(
			await reduce(array, (collector, item) => [collector, item].join("")),
			"Should be an array of results"
		).to.equal("AAA"));

	it("Should use passed initial value", async () =>
		expect(
			await reduce(array, (collector, item) => [collector, item].join(""), "B"),
			"Should be an array of results"
		).to.equal("BAAA"));
	...
	it("Should wait for async reducer functions as well", async () =>
		expect(
			await reduce(
				...
				(collector, item) =>
					new Promise(resolve =>
						setTimeout(() => resolve([collector, item].join("")), 40)
					),
				"B"
			),
			"Should be an array of results"
	...
	it("Should pass the index as third argument to reducer", async () => {
		...
		await reduce(array, (collector, item, index) => results.push(index), {});
	...
	it("Should pass the array of results as fourth argument to reducer", async () =>
		await reduce(array, (collector, item, index, arr) => arr === "AAA"));
