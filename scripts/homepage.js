...
const downloadcount = name =>
	`fetch('https://api.npmjs.org/downloads/point/last-week/${name}').then(result=>result.json()).then(({downloads}) => {window['downloadcount${name}'].innerText = downloads || '0'}).catch(error => { /* ignore */ });`;

start();
async function start() {
	const [rows, links, scripts] = [[], [], []];

	(await readdir("packages")).forEach(item => {
		if (item.startsWith(".")) {
			return;
		}

		const pkg = require(`../packages/${item}/package.json`);
		const { name, description, version, browser, bin } = pkg;
...
`);
	...
	const bottom = ["<script defer>", ...scripts, "</script>"].join("\n");

	const template = (await readFile("src/homepage.html")).toString();
	const changed = `${await git.author}: "<a href="https://github.com/omrilotan/mono/commit/${await git.sha}"><b>${await git.message}</b></a>" <small>on ${dateformat(
		await git.date,
		"dddd, mmmm dS, yyyy"
	)}</small>`;
